...
- Tag pushes (`v*`) in `.github/workflows/build.yaml` now create a GitHub Release automatically after image/chart builds. The release notes include Trivy vulnerability summaries (severity totals + top HIGH/CRITICAL findings) for backend, spoke, and namespace-resolver images, and attach raw Trivy JSON reports.
