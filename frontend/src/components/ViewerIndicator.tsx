...
interface ViewerIndicatorProps {
  viewers: { user_id: string; username: string; display_name: string }[];
}
...
export function ViewerIndicator({ viewers }: ViewerIndicatorProps) {
  ...
  const names = viewers.map((v) => v.display_name).join(", ");

  return (
    <Tooltip content={t("presence.tooltip", { users: names })}>
      <Label color="blue" icon={<EyeIcon />}>
        {viewers.length === 1
          ? t("presence.viewing_one", { user: viewers[0].display_name })
